...
export async function sendJoinRequestFlex(
  to,
  { requesterName, requesterRole, houseName, houseAdminName, liffUrl }
) {
  if (!to) return;
  ...
  const bodyContents = [
    {
      ...
      contents: [
        {
          type: 'text',
          text: isAdminRequester ? 'แอดมิน' : 'ผู้ใช้',
          color: '#aaaaaa',
          size: 'sm',
          flex: 3,
        },
  ...
  if (isAdminRequester) {
    bodyContents.push({
      ...
      spacing: 'sm',
      contents: [
        { type: 'text', text: 'แอดมินบ้าน', color: '#aaaaaa', size: 'sm', flex: 3 },
        {
          type: 'text',
          text: houseAdminName,
          wrap: true,
  ...
  const flexMessage = {
    type: 'flex',
    altText: `🏠 มีคำขอเข้าร่วมบ้านแชร์: ${requesterName}`,
    contents: {
      type: 'bubble',
      header: {
        type: 'box',
        ...
        contents: [
          {
            type: 'text',
            text: 'ผู้ต้องการเข้าร่วมบ้านแชร์',
            weight: 'bold',
            color: '#ffffff',
            size: 'sm',
            align: 'center',
        ...
        contents: [
          {
            ...
            height: 'sm',
            color: '#38a169',
            action: { type: 'uri', label: 'ตรวจสอบ', uri: liffUrl },
          },
        ],
      },
    },
  ...
  await pushMessage(to, [flexMessage]).catch((e) => console.error('sendJoinRequestFlex error:', e));
}
