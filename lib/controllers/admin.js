...
export async function getAdminDashboard(data) {
  ...
  try {
    ...
    const { data: houseMembers, error: e2 } = await supabaseAdmin
      .from('member_houses')
      .select('*, member:member_id(id, name, nickname, phone, role, status), bank:assigned_bank_id(id, bank_name, account_no, account_name)')
      .eq('admin_id', caller_id)
  ...
  try {
    ...
    if (new_status === 'ACTIVE') {
      await supabaseAdmin
        ...
        .eq('id', house.member_id);
    }

    return { status: 'success', message: new_status === 'ACTIVE' ? 'อนุมัติสมาชิกเรียบร้อย!' : 'บล็อคสมาชิกเรียบร้อย' };
  } catch (error) {
    throw error;
  }
}
...
export async function assignMemberBank(data) {
  ...
  try {
    ...
    if (error) throw error;
    return { status: 'success', message: bank_id ? 'กำหนดบัญชีเรียบร้อย' : 'รีเซ็ตเป็นบัญชีหลักเรียบร้อย' };
  } catch (error) {
    throw error;
  }
}
