...
export async function approveHouseMember(data) {
  ...
  try {
    ...
    // Sync the global members.status to mirror this house decision so that
    // BLOCKED members are restricted across the whole app (cannot create new
    // circles, cannot reserve hands, cannot request to join other houses).
    if (new_status === 'ACTIVE' || new_status === 'BLOCKED') {
      await supabaseAdmin.from('members').update({ status: new_status }).eq('id', house.member_id);
    }
...
export async function getAdminCircles(data) {
  ...
  try {
    ...
    if (targetIds.length > 0) {
      ...
      nicknameByTarget = new Map((nicknameRows || []).map((r) => [r.target_id, r.nickname]));
    }
...
export async function removeHouseMember(data) {
  ...
  try {
    // Get the house record to find member_id
    ...
    const memberId = house.member_id;

    // Check if member has ACTIVE circles in this house
    const { data: activeCircles } = await supabaseAdmin
      .from('circles')
      .select('id')
      .eq('creator_id', caller_id)
      .eq('status', 'ACTIVE');

    if (activeCircles && activeCircles.length > 0) {
      const activeCircleIds = activeCircles.map((c) => c.id);
      ...
      if (activePlayers && activePlayers.length > 0) {
        return { status: 'error', message: 'ไม่สามารถลบได้ สมาชิกกำลังเล่นวงแชร์ในรอบปัจจุบัน' };
      }
    }

    // Remove OPEN circle hand reservations for this member in this house
    ...
    if (openCircles && openCircles.length > 0) {
      const openCircleIds = openCircles.map((c) => c.id);
      await supabaseAdmin
        .from('circle_players')
        .delete()
        .eq('member_id', memberId)
    ...
    // Delete the member_houses record
    const { error } = await supabaseAdmin
      .from('member_houses')
      .delete()
      .eq('id', house_id)
...
export async function fullDeleteMember(data) {
  ...
  try {
    // 1) Reject if member runs an ACTIVE circle in their own house
    const { data: ownActive } = await supabaseAdmin
      .from('circles')
      .select('id')
      .eq('creator_id', member_id)
      .eq('status', 'ACTIVE')
      .limit(1);
    if (ownActive && ownActive.length > 0) {
      return {
        status: 'error',
        message: 'ไม่สามารถลบออกจากแอปได้ สมาชิกท่านนี้มีวงแชร์ที่กำลังเล่นอยู่ในบ้านแชร์ของตัวเอง',
      };
    }

    // 2) Reject if member is a LIVE player in any ACTIVE circle (other houses)
    const { data: liveSeats } = await supabaseAdmin
      .from('circle_players')
      .select('circle_id, status, circle:circles!circle_id(status)')
      .eq('member_id', member_id)
      .eq('status', 'LIVE');
    const playingActive = (liveSeats || []).some((s) => s.circle?.status === 'ACTIVE');
    if (playingActive) {
      return {
        status: 'error',
        message: 'ไม่สามารถลบออกจากแอปได้ สมาชิกท่านนี้กำลังเล่นวงแชร์อยู่',
      };
    }
...
export async function updateMemberRole(data) {
  ...
  try {
    // Block role change if target currently runs an ACTIVE circle in their own house.
    // (Their role determines house ownership; flipping it mid-game would break things.)
    const { data: ownActiveCircles } = await supabaseAdmin
      .from('circles')
      .select('id')
      .eq('creator_id', member_id)
      .eq('status', 'ACTIVE')
      .limit(1);
    if (ownActiveCircles && ownActiveCircles.length > 0) {
      return {
        status: 'error',
        message: 'ไม่สามารถเปลี่ยนยศได้ ขณะมีวงแชร์ที่กำลังเล่นอยู่ในบ้านแชร์ของสมาชิกท่านนี้',
      };
    }
