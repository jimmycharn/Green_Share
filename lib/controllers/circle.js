...
export async function createCircle(data) {
  try {
    ...
    if (newCircleData.type !== 'ขั้นบันได (ดอกคงที่)') {
      ...
      console.log('Step-interest circle created — skipping auto LINE notification. Admin must set period amounts first.');
    }
...
export async function getCircles(data) {
  try {
    ...
    if (member.role === 'SUPERADMIN') {
      ...
      if (joinedSet.size > 0) {
        queries.push(
          supabaseAdmin.from('circles').select('*').in('id', Array.from(joinedSet)),
        );
      }
...
export async function getCircleDetail(data) {
  try {
    ...
    const [playersRes, bidsRes, slipsRes, houseLinkRes, payoutsRes, assignmentsRes, periodDatesRes] =
      await Promise.all([
...
export async function updateCircleSettings(data) {
  try {
    ...
    if (data.period_config) {
      ...
      if (amount !== undefined) updateDatePayload.amount = parseFloat(amount) || 0;
      ...
      if (Object.keys(updateDatePayload).length > 0) {
        await supabaseAdmin
          .from('circle_period_dates')
          .update(updateDatePayload)
          .eq('circle_id', data.circle_id)
...
export async function updatePeriodDate(data) {
  try {
    ...
    const { error } = await supabaseAdmin.from('circle_period_dates').upsert(
      updatePayload,
      { onConflict: 'circle_id,period' }
    );
