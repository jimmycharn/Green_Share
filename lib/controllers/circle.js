...
export async function createCircle(data) {
  try {
    ...
    try {
      // Find all active members in this admin's house
      const { data: houseMembers } = await supabaseAdmin
        .from('member_houses')
        .select('member_id, members(line_id)')
        .eq('admin_id', data.creator_id)
        .eq('status', 'ACTIVE');

      console.log(`Found ${houseMembers?.length || 0} active members to notify.`);
      ...
      // Always notify the admin (the creator) as well
      if (process.env.ADMIN_LINE_UID) {
        const adminAlreadyNotified = houseMembers?.some(hm => hm.members?.line_id === process.env.ADMIN_LINE_UID);
        if (!adminAlreadyNotified) {
          await sendCircleFlex(process.env.ADMIN_LINE_UID, newCircleData);
        }
      }
    } catch (lineErr) {
      console.error("LINE Group Notify Error:", lineErr);
    }
