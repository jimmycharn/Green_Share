...
export async function updateCircleSettings(data) {
  try {
    ...
    const lockedFields = ['amount_per_hand', 'total_hands', 'type', 'period_type', 'period_interval', 'period_value', 'start_date'];
    const attemptedLocked = lockedFields.filter((f) => data[f] !== undefined);
    if (circle && (circle.status === 'ACTIVE' || circle.status === 'CLOSED') && attemptedLocked.length > 0) {
      return {
        status: 'error',
