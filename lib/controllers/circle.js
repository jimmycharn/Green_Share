...
export async function createCircle(data) {
  try {
    // 1. Get total circles to generate ID (with Error check)
    ...
    if (countError) {
      ...
      return { status: 'error', message: `ดึงข้อมูลจำนวนวงล้มเหลว: ${countError.message}` };
    }
    
    const newId = generateNextId('C', count || 0);

    // 2. Prepare assignments (Default to creator/admin)
    const initialAssignments = {};
    for (let i = 1; i <= data.total_hands; i++) {
        initialAssignments[i] = data.creator_id;
    }

    // 3. Insert into database
    const { error: insertError } = await supabaseAdmin.from('circles').insert([{
      id: newId,
    ...
    // 4. Notify admin (Don't let this crash the success response)
    try {
