...
export async function createCircle(data) {
  try {
    ...
    const { error } = await supabaseAdmin.from('circles').insert([{
      ...
      min_bid: parseInt(data.min_bid) || 0,
      max_bid: parseInt(data.max_bid) || 500,
      close_mode: data.close_mode || 'MANUAL',
      auto_action: data.auto_action || 'NOTIFY'
    }]);

    if (error) throw error;

    await notifyAdmin(`✨ เปิดวงใหม่!\nชื่อวง: ${data.circle_name}\nส่งงวดละ: ${data.amount_per_hand}`);
...
export async function getCircles(data) {
  try {
    // Basic verification: user is active
    const { data: member } = await supabaseAdmin.from('members').select('status').eq('id', data.member_id).single();
    ...
    // Filter circles user is part of (if needed) or just return open circles
...
export async function getCircleDetail(data) {
  try {
    ...
    const { data: bids } = await supabaseAdmin.from('bids').select('*').eq('circle_id', data.circle_id);
