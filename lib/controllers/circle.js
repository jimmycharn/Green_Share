...
export async function createCircle(data) {
  try {
    // Reject if creator is BLOCKED globally
    ...
    if (creator?.status === 'BLOCKED') {
      return { status: 'error', message: 'บัญชีของคุณถูกบล็อก ไม่สามารถสร้างวงแชร์ใหม่ได้' };
    }
    ...
    const newCircleData = {
      ...
      fee_per_hand: parseFloat(data.fee_per_hand) || 0,
    };
    ...
    // Also create a player record for hand_no 1 so it shows in the members tab
    ...
    // Insert creator as player for hand_no 1 (visible in members tab)
    const { data: creatorMember } = await supabaseAdmin
      .from('members')
      .select('name')
      .eq('id', data.creator_id)
      .single();
    await supabaseAdmin.from('circle_players').insert([
      {
        circle_id: newId,
        hand_no: 1,
        member_id: data.creator_id,
        member_name: creatorMember?.name || 'ท้าวแชร์',
        status: 'LIVE',
      },
    ]);
...
export async function getCircles(data) {
  try {
    ...
    // BLOCKED members can still view and play their existing ACTIVE circles.
    // Only PENDING (not yet approved) members are blocked from seeing anything.
    ...
    // Resolve allowed creator ids: user's own house (if admin) + houses they are ACTIVE member of
    ...
    const { data: houses, error: hErr } = await supabaseAdmin
      .from('member_houses')
      .select('admin_id, admin:members!admin_id(role)')
      .eq('member_id', member.id)
      .eq('status', 'ACTIVE');
    if (hErr) throw hErr;
    for (const h of houses || []) {
      if (!h.admin_id) continue;
      // Skip admin-registration links: when an ADMIN registers, a member_houses row
      // is created linking them to the SUPERADMIN. That's NOT a real "joined house"
      // for playing circles, so exclude it from circle visibility.
      if (member.role === 'ADMIN' && h.admin?.role === 'SUPERADMIN') continue;
      allowedCreatorIds.add(h.admin_id);
    }
    ...
    if (allowedCreatorIds.size > 0) {
      const { data, error } = await supabaseAdmin
        .from('circles')
        .select('*')
        .in('creator_id', Array.from(allowedCreatorIds))
    ...
    // Also include circles the member personally joined as a player
    // (critical for BLOCKED members who can still play their existing ACTIVE circles)
    const circlesById = new Map(circlesData.map((c) => [c.id, c]));
    const joinedCircleIds = Array.from(joinedSet).filter((id) => !circlesById.has(id));
    if (joinedCircleIds.length > 0) {
      const { data: joinedCircles } = await supabaseAdmin
        .from('circles')
        .select('*')
      ...
      circlesData = Array.from(circlesById.values());
    }

    // Fetch creator names/pictures for the circles
    const creatorIds = [...new Set(circlesData.map((c) => c.creator_id).filter(Boolean))];
    ...
    if (creatorIds.length > 0) {
      ...
      for (const c of creators || []) {
        creatorMap.set(c.id, { name: c.name, picture_url: c.picture_url });
      }
    }
    ...
    return {
      ...
      circles: circlesData.map((c) => {
        ...
        return {
          ...
          creator_name: creator.name || 'ท้าวแชร์',
          creator_picture_url: creator.picture_url || null,
        };
      }),
    };
  } catch (error) {
...
export async function getCircleDetail(data) {
  try {
    ...
    const realPlayers = (playersRes.data || []).map(({ members: m, ...p }) => ({
      ...p,
      picture_url: m?.picture_url || null,
    }));

    // For stair circles, some hands may be pre-assigned via circle_period_assignments
    // without a circle_players record. Synthesize virtual players so every viewer
    // sees the assigned member, even if they are from a different house.
    const assignedMemberIds = [...new Set(Object.values(assignments))].filter(Boolean);
    ...
    if (assignedMemberIds.length > 0) {
      ...
      for (const m of assignedMembers || []) {
        assignedMembersMap[m.id] = m;
      }
    }

    const realPlayerHandNos = new Set(realPlayers.map((p) => p.hand_no));
    const players = [...realPlayers];
    for (const [period, memberId] of Object.entries(assignments)) {
      const handNo = parseInt(period, 10);
      if (!realPlayerHandNos.has(handNo) && memberId && memberId !== 'NONE') {
        const m = assignedMembersMap[memberId];
        players.push({
          id: `virtual-${handNo}`,
          hand_no: handNo,
          member_id: memberId,
          member_name: m?.name || 'ท้าวแชร์',
          picture_url: m?.picture_url || null,
          status: 'LIVE',
          interest_earned: 0,
...
export async function joinCircle(data) {
  try {
    ...
    if (member.status === 'BLOCKED') {
      return { status: 'error', message: 'สมาชิกถูกบล็อก ไม่สามารถจองมือได้' };
    }
...
export async function verifySlip(data) {
  try {
    ...
    const slipStatus = data.slip_status || 'APPROVED';
    const { error } = await supabaseAdmin
      .from('slips')
    ...
    return {
      ...
      message:
        slipStatus === 'APPROVED'
          ? 'อนุมัติเรียบร้อย'
          : 'ไม่อนุมัติสลิปแล้ว สมาชิกต้องส่งหลักฐานใหม่',
    };
  } catch (error) {
    throw error;
  }
...
export async function randomSelectBidder(data) {
  try {
    ...
    // 4. Check if there are existing bids for this period (tie case)
    const { data: currentPeriodBids } = await supabaseAdmin
      .from('bids')
      .select('member_id, bid_amount, bid_time, member:members!member_id(name)')
      .eq('circle_id', circle_id)
      .eq('period', period)
      .order('bid_amount', { ascending: false });

    let pool;
    let isFromBids = false;

    if (currentPeriodBids && currentPeriodBids.length > 0) {
      const toSec = (t) => new Date(t).toISOString().substring(0, 19);
      const topAmount = currentPeriodBids[0].bid_amount;
      const topSec = toSec(currentPeriodBids[0].bid_time);
      // True tie: same amount AND same second
      const tiedBids = currentPeriodBids.filter(
        (b) => b.bid_amount === topAmount && toSec(b.bid_time) === topSec
      );
      if (tiedBids.length === 1) {
        return { status: 'error', message: 'มีผู้ชนะชัดเจนอยู่แล้ว ไม่จำเป็นต้องสุ่ม' };
      }
      pool = tiedBids.map((b) => ({
        id: b.member_id,
        name: b.member?.name || b.member_id,
    ...
    if (!pool || pool.length === 0) {
      return { status: 'error', message: 'ไม่มีผู้ที่สามารถสุ่มได้' };
    }

    // 5. Randomly pick from pool
    const luckyWinner = pool[Math.floor(Math.random() * pool.length)];

    // 6. Insert/upsert bid for them (amount 0 if from eligible pool, preserve if from tie pool)
    const { error: bidErr } = await supabaseAdmin.from('bids').upsert(
      {
        circle_id,
        period,
        member_id: luckyWinner.id,
        bid_amount: isFromBids ? luckyWinner.bid_amount : 0,
        bid_time: new Date(),
      },
      { onConflict: 'circle_id, period, member_id' }
    );
...
export async function updateCircleSettings(data) {
  try {
    ...
    // Fetch circle to check status and prevent locked-field edits after ACTIVE
    const { data: circle } = await supabaseAdmin
      .from('circles')
      .select('status')
      .eq('id', data.circle_id)
    ...
    const attemptedLocked = lockedFields.filter((f) => data[f] !== undefined);
    if (
      circle &&
      (circle.status === 'ACTIVE' || circle.status === 'CLOSED') &&
      attemptedLocked.length > 0
    ) {
      return {
        status: 'error',
        message: `ไม่สามารถแก้ไข ${attemptedLocked.join(', ')} ได้เนื่องจากวงแชร์เริ่มดำเนินการไปแล้ว`,
      };
    }
    ...
    if (data.period_config) {
      ...
      if (
        data.period_config.fee_per_hand !== undefined &&
        data.period_config.fee_per_hand !== null &&
        data.period_config.fee_per_hand !== ''
      ) {
        updateDatePayload.fee_per_hand = parseFloat(data.period_config.fee_per_hand) || 0;
      }
    ...
    if (typeof data.fee_per_hand !== 'undefined') {
      updatePayload.fee_per_hand = parseFloat(data.fee_per_hand) || 0;
    }
...
export async function notifyBidStart(data) {
  try {
    ...
    // Fetch all live players in this circle with their line_ids
    const { data: players, error: playersErr } = await supabaseAdmin
      .from('circle_players')
      .select('member_id, member:members!member_id(line_id)')
      .eq('circle_id', data.circle_id)
      .eq('status', 'LIVE');
...
export async function createPayout(data) {
  try {
    ...
    // Fetch circle and winner's hand count to calculate management fee
    const [circleRes, handsRes, periodFeeRes] = await Promise.all([
      supabaseAdmin.from('circles').select('fee_per_hand, name').eq('id', data.circle_id).single(),
      supabaseAdmin
        .from('circle_players')
        .select('hand_no')
        .eq('circle_id', data.circle_id)
    ...
    const circleFee = circleRes.data?.fee_per_hand || 0;
    const periodFee = periodFeeRes.data?.fee_per_hand;
    const effectiveFeePerHand =
      periodFee !== null && periodFee !== undefined ? periodFee : circleFee;
    const winnerHandsCount = (handsRes.data || []).length;
    ...
    const { data: memberData } = await supabaseAdmin
      ...
      .single();
    if (circleRes.data && memberData?.line_id) {
      sendPayoutNotificationToMember(memberData.line_id, {
        circleName: circleRes.data.name,
        period: data.period,
...
export async function removeEmptyHand(data) {
  try {
    if (!['SUPERADMIN', 'ADMIN'].includes(data.caller_role)) {
      return { status: 'error', message: 'ไม่มีสิทธิ์ลบมือ' };
    }

    const handNo = parseInt(data.hand_no);
    const circleId = data.circle_id;

    // 1. Verify circle is OPEN
    const { data: circle, error: circleErr } = await supabaseAdmin
      .from('circles')
      .select('status, total_hands')
      .eq('id', circleId)
      .single();
    ...
    if (circle.status !== 'OPEN') {
      return {
        status: 'error',
        message: 'สามารถลบมือได้เฉพาะวงแชร์ที่ยังไม่ได้เริ่ม (OPEN) เท่านั้น',
      };
    }

    // 2. Verify the hand is empty (no player in circle_players)
    ...
    if (existingPlayer) {
      return { status: 'error', message: 'ไม่สามารถลบมือที่มีสมาชิกจองไว้แล้วได้' };
    }

    // 3. Also check circle_period_assignments (pre-assigned but not yet a player)
    ...
    if (existingAssignment) {
      // Delete the assignment for the empty slot before shifting
      await supabaseAdmin
        .from('circle_period_assignments')
        .delete()
        .eq('circle_id', circleId)
    ...
    // 4. Delete period_dates for this slot
    await supabaseAdmin
      .from('circle_period_dates')
      .delete()
      .eq('circle_id', circleId)
    ...
    // 5. Shift circle_period_dates: period > handNo → period - 1
    // Fetch rows to shift, then re-insert with updated periods
    const { data: datesToShift } = await supabaseAdmin
      .from('circle_period_dates')
      .select('*')
      .eq('circle_id', circleId)
      .gt('period', handNo)
    ...
    if (datesToShift && datesToShift.length > 0) {
      ...
      // Re-insert with period shifted down
      const shifted = datesToShift.map(({ id: _id, ...row }) => ({
        ...row,
        period: row.period - 1,
      }));
      await supabaseAdmin.from('circle_period_dates').insert(shifted);
    }

    // 6. Shift circle_period_assignments: period > handNo → period - 1
    const { data: assignsToShift } = await supabaseAdmin
      .from('circle_period_assignments')
      .select('*')
      .eq('circle_id', circleId)
      .gt('period', handNo)
    ...
    if (assignsToShift && assignsToShift.length > 0) {
      await supabaseAdmin
        .from('circle_period_assignments')
        .delete()
        .eq('circle_id', circleId)
      ...
      const shiftedAssigns = assignsToShift.map(({ id: _id, ...row }) => ({
        ...row,
        period: row.period - 1,
      }));
      await supabaseAdmin.from('circle_period_assignments').insert(shiftedAssigns);
    }

    // 7. Shift circle_players: hand_no > handNo → hand_no - 1
    const { data: playersToShift } = await supabaseAdmin
      .from('circle_players')
      .select('id, hand_no')
      .eq('circle_id', circleId)
      .gt('hand_no', handNo)
    ...
    if (playersToShift && playersToShift.length > 0) {
      for (const p of playersToShift) {
        await supabaseAdmin
          .from('circle_players')
          .update({ hand_no: p.hand_no - 1 })
          .eq('id', p.id);
      }
    }

    // 8. Decrement total_hands on the circle
    const { error: updateErr } = await supabaseAdmin
      .from('circles')
      .update({ total_hands: circle.total_hands - 1 })
      .eq('id', circleId);
    ...
    return {
      status: 'success',
      message: `ลบมือที่ ${handNo} เรียบร้อย มือที่เหลือถูกเลื่อนขึ้นมาแทน`,
    };
  } catch (error) {
    throw error;
  }
...
export async function notifyUnpaidMembers(data) {
  try {
    if (!['SUPERADMIN', 'ADMIN'].includes(data.caller_role)) {
      return { status: 'error', message: 'ไม่มีสิทธิ์ส่งการแจ้งเตือน' };
    }
    ...
    // Get all players (members with hands)
    ...
    const { data: approvedSlips } = await supabaseAdmin
      .from('slips')
    ...
    if (isStairType) {
      const { data: assignment } = await supabaseAdmin
        ...
        .eq('period', period)
        .maybeSingle();
      winnerId = assignment?.member_id || null;
    } else {
      const { data: periodBids } = await supabaseAdmin
        .from('bids')
        .select('member_id, bid_amount')
        .eq('circle_id', data.circle_id)
        .eq('period', period)
        .order('bid_amount', { ascending: false });
      winnerId = periodBids?.[0]?.member_id || null;
    }

    // Identify unpaid members
    const approvedMemberIds = new Set((approvedSlips || []).map((s) => s.member_id));
    const uniqueMemberIds = [...new Set((players || []).map((p) => p.member_id))];
    const unpaidMemberIds = uniqueMemberIds.filter((mId) => {
      if (mId === winnerId) return false; // winner doesn't pay
      return !approvedMemberIds.has(mId);
    });

    if (unpaidMemberIds.length === 0) {
      return { status: 'success', message: 'สมาชิกทุกคนจ่ายครบแล้ว ไม่มีผู้ที่ยังไม่จ่าย' };
    }

    // Get member details with LINE IDs
    const { data: members } = await supabaseAdmin
      .from('members')
      .select('id, name, line_id')
      .in('id', unpaidMemberIds);

    // Get period date for due date display
    const { data: pDate } = await supabaseAdmin
      .from('circle_period_dates')
      .select('period_date')
      .eq('circle_id', data.circle_id)
      .eq('period', period)
    ...
    const dueDate = pDate?.period_date
      ? new Date(pDate.period_date).toLocaleDateString('th-TH')
      : null;

    // Send reminders
    const { sendPaymentReminder } = await import('@/lib/line.js');
    let sent = 0;
    for (const m of members || []) {
      if (m.line_id) {
        const handsCount = (players || []).filter((p) => p.member_id === m.id).length;
        const amount = handsCount * (circle.amount_per_hand || 0);
        await sendPaymentReminder(m.line_id, {
          circleName: circle.name,
          period,
          amount,
          dueDate,
    ...
    return {
      status: 'success',
      message: `ส่งแจ้งเตือนให้ผู้ที่ยังไม่จ่าย ${sent} คนเรียบร้อยแล้ว`,
    };
  } catch (error) {
    throw error;
  }
