...
export async function createCircle(data) {
  try {
    ...
    try {
      ...
      console.log(`Found ${houseMembers?.length || 0} members to notify for Admin ID: ${data.creator_id}`);
      ...
      if (process.env.ADMIN_LINE_UID) {
        const adminInList = houseMembers?.some(hm => hm.member?.line_id === process.env.ADMIN_LINE_UID);
        if (!adminInList) {
