...
export async function registerMember(data) {
  const { line_id, name, nickname, phone, bank_account, role, house_name, house_code, picture_url } = data;
  ...
  try {
    ...
    let query = supabaseAdmin
      ...
      .select(
        ...
        member_houses!member_id(id, admin_id, status, assigned_bank_id, custom_nickname)
      `
      )
    ...
    const membersWithHouseStatus = members.map((m) => {
      ...
      // Only expose `custom_nickname` from the row that belongs to the caller
      // (i.e., where the caller is the admin). This keeps each admin's custom
      // nicknames private to them — regular members never see them.
      const myHouseInfo = m.member_houses?.find((h) => h.admin_id === requester.id);
      return {
        ...m,
        house_id: houseInfo?.id,
        house_status: houseInfo?.status || 'NOT_JOINED',
        assigned_bank_id: houseInfo?.assigned_bank_id ?? null,
        custom_nickname: myHouseInfo?.custom_nickname ?? null,
      };
    });
