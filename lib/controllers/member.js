...
export async function registerMember(data) {
  try {
    ...
    // Optimization: Skip if no house param or if it's their own house
...
export async function getMembers(data) {
  try {
    ...
    // Filter or map to show house-specific status if needed
    const membersWithHouseStatus = members.map(m => {
      const houseInfo = m.member_houses?.find(h => h.admin_id === data.member_id);
      return {
        ...m,
