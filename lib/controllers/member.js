...
export async function completeOnboarding(data) {
  ...
  try {
    ...
    let targetAdminId = role === 'ADMIN' ? house_code : house_code; // Default
    
    // Find Superadmin for house creation request or role check
    ...
    if (role === 'ADMIN') {
      ...
      // If invited by Superadmin, upgrade to ADMIN role as per user requirement
      if (house_code === superAdmin?.id) {
         await supabaseAdmin.from('members').update({ role: 'ADMIN' }).eq('id', member_id);
      }
    }
