...
export default function Members() {
  ...
  const handleDeleteMember = async (targetMemberId, targetName) => {
    if (!window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบคุณ ${targetName} ออกจากระบบโดยสมบูรณ์?`))
      return;
    ...
    <div className="animate-fade-in">
      ...
      {isAdmin && activeTab === 'my_house' && (
        <div
          ...
          style={{ textAlign: 'center', marginBottom: '32px', border: '1px dashed var(--primary)' }}
        >
          <div style={{ fontSize: '2rem', marginBottom: '12px' }}>🤝</div>
          <h3 style={{ marginBottom: '8px', fontSize: '1.2rem', fontWeight: '700' }}>
            ชวนเพื่อนเข้าบ้าน
          </h3>
          <p
            style={{
      ...
      {isLoadingMembers ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <h3
            style={{ fontSize: '1.1rem', fontWeight: '800', marginBottom: '8px', color: '#1e293b' }}
          >
            ท้าวแชร์
          </h3>
          {myHouseAdmin ? (
            <MemberCard
              member={myHouseAdmin}
              isSelf={myHouseAdmin.id === dbUser.id}
              dbUser={dbUser}
            />
          ) : (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          >
            ท้าวแชร์ท่านอื่น ({otherAdmins.length})
          </h3>
            ...
            <div key={admin.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div
                    style={{
                      fontSize: '1.2rem',
                      transform: expandedAdmin === admin.id ? 'rotate(180deg)' : 'rotate(0deg)',
                      transition: '0.3s',
                      marginLeft: '10px',
                    }}
                  >
              ...
              {expandedAdmin === admin.id && (
                ...
                >
                  ...
                  {getMembersByAdmin(admin.id).length === 0 && (
                    <div
                      style={{
                        fontSize: '0.9rem',
                        color: '#64748b',
...
function MemberCard({
  ...
  return (
    ...
    >
      <div
        style={{
          width: mini ? '40px' : '50px',
          height: mini ? '40px' : '50px',
          borderRadius: '15px',
          background: isAdmin ? 'var(--primary-gradient)' : '#1e293b',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          fontSize: mini ? '1.1rem' : '1.3rem',
      ...
      <div style={{ flex: 1 }}>
        <div
          style={{ fontWeight: '800', fontSize: mini ? '0.95rem' : '1.05rem', color: '#1e293b' }}
        >
      ...
      <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          >
            ...
            <span
              style={{
                fontSize: '0.75rem',
                fontWeight: '800',
                color: isAdmin ? 'var(--primary)' : '#94a3b8',
              }}
            >
        ...
        {!isSelf && ['SUPERADMIN', 'ADMIN'].includes(dbUser?.role) && (
          <div style={{ display: 'flex', gap: '8px' }}>
            {isPending && (
              <button
                onClick={() => handleApprove(member.house_id, member.name)}
